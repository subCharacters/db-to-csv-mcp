...
- 복잡한 로직은 명시적으로 분리한다. 3단 이상의 `if` 중첩은 금지. 복잡한 분기문은 전략 패턴 혹은 Enum으로 분리한다.
- `static`과 `singleton`은 최소화한다.
- DB, API, 파일 등 외부 자원 접근은 예외처리와 타임아웃을 갖는다.
- 모든 입력은 명시적으로 검증한다. `@NotNull` 등의 Validation을 적극 활용.
- 재귀 호출 대신 명시적 반복을 사용한다.
- 런타임에 직접 생성(`new`)하는 리소스를 최소화하고, 의존성 주입(DI)으로 관리한다.
- `switch`문은 반드시 `default`를 갖는다.
- 모든 컴파일 및 테스트 경고는 에러로 간주하고, 반드시 0건을 유지한다.

### 코딩 규칙2
- 함수는 하나의 일만 한다.
- 변수명과 메서드명은 무엇을 하는지 드러나야한다.
- 클래스명은 무슨 일을 하는지 드러나야한다.
- 예외는 의미있는 메시지를 포함.(왜 실패하였는가)
- 주석은 무엇을 하는지가 아니라 왜 이렇게 하였는가를 설명한다.
- 1개의 클래스는 1개의 책임을 갖는다.
- 테스트 코드 또한 동일한 품질 규칙을 적용한다.

### 코딩 규칙3
- 클래스는 단일 책임을 갖는다. (SRP)
- 확장은 자유롭고 변경에는 닫혀 있어야 한다. (OCP)
- 하위 클래스는 상위 클래스의 계약을 위반하지 않아야 한다. (LSP)
- 인터페이스는 역할 단위로 작게 분리한다. (ISP)
- 구현보다 추상화(인터페이스/추상클래스)에 의존한다. (DIP)
